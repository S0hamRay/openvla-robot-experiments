...
# Removed circular import - these functions will be imported directly where needed
...
def get_action(cfg, model, obs, task_label, processor=None):
    ...
    Returns:
        - action (np.ndarray)
        - sparse_code (np.ndarray) if SAE is used, else None
    """
    if cfg.model_family == "openvla":
        # Import here to avoid circular import
        ...
        if isinstance(result, tuple) and len(result) == 2:
            action, sparse_code = result
            assert action.shape == (ACTION_DIM,)
            return action, sparse_code
        else:
            action = result
            assert action.shape == (ACTION_DIM,)
            return action, None
