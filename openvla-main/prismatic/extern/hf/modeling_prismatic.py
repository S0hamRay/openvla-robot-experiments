...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def __init__(self, config: OpenVLAConfig, sparse_autoencoder: SparseAutoencoderTorch = None) -> None:
        super().__init__(config)
    ...
    def predict_action(
        ...
        If self.sparse_autoencoder is not None, returns a dict with both 'actions' and 'sparse_code'.
        """
        ...
        if self.sparse_autoencoder is not None:
            # Ensure actions_tensor is on the same device as the SAE
            sae_device = self.sparse_autoencoder.encoder.weight.device
            actions_tensor = torch.tensor(actions, dtype=torch.float32, device=sae_device)
            if actions_tensor.ndim == 1:
                actions_tensor = actions_tensor.unsqueeze(0)
            sparse_code = self.sparse_autoencoder.encode(actions_tensor).cpu().detach().numpy()
            return {"actions": actions, "sparse_code": sparse_code}
